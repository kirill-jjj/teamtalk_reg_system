...
3.  **Установите зависимости и настройте окружение с помощью `uv`:**
    ...
    uv sync
    ```
    Чтобы включить зависимости для разработки (например, `pytest` для тестирования), используйте:
    ```bash
    uv sync --dev
    ```
    Это гарантирует, что в вашем окружении будут все необходимые пакеты как для запуска приложения, так и для его разработки/тестирования.
...
4.  **Скомпилируйте файлы локализации**
    ...
    Активируйте ваше виртуальное окружение (например, `source .venv/bin/activate` для Linux/macOS или `.\.venv\Scripts\activate` для Windows, если оно еще не активно). Затем выполните следующую команду из корневого каталога проекта:
    ```bash
...
Инструкции по запуску тестов для FastAPI приложения смотрите в файле `bot/fastapi_app/TESTING.md`.
