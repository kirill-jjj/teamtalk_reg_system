...
5.  **Настройте бота:**
    *   Проект теперь использует файл `config.toml` для конфигурации. Файл `config.toml` создается по умолчанию. Вы должны просмотреть его и указать свои данные.
    *   Откройте `config.toml` в текстовом редакторе и укажите ваши данные. Ключевые настройки:
        *   `TG_BOT_TOKEN`: Токен вашего Telegram-бота.
        *   `ADMIN_IDS`: Список ID пользователей Telegram, которые являются администраторами бота (например, `[12345, 67890]`).
        *   Данные для подключения к серверу TeamTalk (`HOST_NAME`, `PORT`, `USER_NAME`, `PASSWORD`).
        *   Настройки веб-регистрации (если включена, используя переменные `WEB_APP_HOST`, `WEB_APP_PORT`).
    *   Вы можете использовать другой файл конфигурации, установив переменную окружения `CONFIG_FILE` (например, `CONFIG_FILE=test.config.toml uv run python run.py`).
...
6.  **Запустите приложение с помощью `uv`:**
    ...
    ```
    Веб-приложение FastAPI будет доступно по хосту и порту, указанным в вашем файле `config.toml` (например, `http://127.0.0.1:5000`).

## Использование

*   **Telegram Бот:** Начните чат с вашим ботом и отправьте команду `/start`.
*   **Веб-регистрация:** Если включена (`WEB_REGISTRATION_ENABLED=true` в вашей конфигурации), перейдите по адресу `http://<ваш_хост>:<ваш_порт>/register` (например, `http://127.0.0.1:5000/register`) в веб-браузере. Хост и порт определяются переменными `WEB_APP_HOST` и `WEB_APP_PORT` в вашем файле конфигурации.
