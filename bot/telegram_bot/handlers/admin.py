...
async def admin_panel_handler(message: types.Message) -> None:
    ...
    if message.from_user.id not in settings.admin_ids:
        logger.warning(
            "User %s (not an admin) tried to use /adminpanel.", message.from_user.id
        )
        return
...
async def delete_user_start_handler(
    ...
    logger.info(
        "Admin %s requested user list for deletion.", callback_query.from_user.id
    )
...
async def confirm_delete_user_handler(
    ...
    if user_reg:
        ...
        logger.info(
            "Found TeamTalk username '%s' for Telegram ID %s before deletion.",
            tt_username_for_ban,
            telegram_id_to_delete,
        )
    else:
        logger.warning(
            "Could not find TelegramRegistration record for Telegram ID %s before deletion. Will ban without TT username.",
            telegram_id_to_delete,
        )
    ...
    if deletion_successful:
        logger.info(
            "Admin %s successfully deleted TelegramRegistration for ID: %s",
            callback_query.from_user.id,
            telegram_id_to_delete,
        )
        ...
        logger.warning(
            "Admin %s failed to delete TelegramRegistration for ID: %s (possibly already deleted or DB error). Ban not applied.",
            callback_query.from_user.id,
            telegram_id_to_delete,
        )
    ...
    try:
        ...
        logger.debug(
            "Could not edit original message after deletion confirmation: %s. The alert was shown.",
            e,
        )
...
async def _build_ban_list_message_and_keyboard(
    ...
    if not banned_users:
        ...
        for buser in banned_users:
            ...
            tt_user_text = (
                buser.teamtalk_username if buser.teamtalk_username else "N/A"
            )
            # Ensure TG ID is a string for formatting if it's not already
            tg_id_str = str(buser.telegram_id)
            message_lines.append(
                f"TG ID: {tg_id_str} - TT User: {tt_user_text} (Reason: {reason_text})"
            )
            builder.button(
...
async def view_ban_list_handler(
    ...
    try:
        ...
        logger.debug(
            "Failed to edit message for ban list view (might be no change or too old): %s",
            e,
        )
...
async def unban_user_handler(
    ...
    if target_id is None:  # Should not happen if buttons are generated correctly
        await callback_query.answer(
            _("Error: No target user ID specified for unban."), show_alert=True
        )
        return
    ...
    alert_text = ""
    if success:
        alert_text = _("User {target_telegram_id} has been unbanned.").format(
        ...
        alert_text = _("Failed to unban user {target_telegram_id}.").format(
            target_telegram_id=target_id
        )
        logger.warning(
...
async def manual_ban_prompt_handler(
    ...
    try:
        await callback_query.message.edit_text(
            _("Please enter the Telegram ID and reason for the ban on separate lines.")
        )
    except Exception as e:
        logger.debug(
            "Could not edit message for manual ban prompt (maybe no change): %s", e
        )
        await callback_query.message.answer(
            _("Please enter the Telegram ID and reason for the ban on separate lines.")
...
async def process_manual_ban_handler(
    ...
    if not parts:  # Should not happen with F.text but good practice
        await message.reply(
            _("Invalid Telegram ID provided.")
    ...
    try:
        ...
        logger.info(
            "Admin %s manually banned user %s with reason: '%s'. TT username: %s",
        ...
        logger.warning(
            "Admin %s provided invalid Telegram ID for manual ban: %s",
            message.from_user.id,
            telegram_id_str,
        )
...
async def list_all_tt_accounts_handler(
    pytalk_bot_instance: pytalk.TeamTalkBot, callback_query: types.CallbackQuery
) -> None:
    ...
    ):  # Changed is_connected() to connected
        logger.warning(
            "list_all_tt_accounts_handler: TeamTalk instance not available or not connected."
        )
        try:
            await callback_query.message.edit_text(
                _(
                    "Could not connect to the TeamTalk server to get the list of accounts."
                ),
                reply_markup=None,
            )
        except Exception as e_edit:
            logger.debug(
                "Failed to edit message for TT connection error: %s", e_edit
            )
            await callback_query.message.answer(
                _(
                    "Could not connect to the TeamTalk server to get the list of accounts."
        ...
        if user_accounts_sdk:
            for acc_sdk in user_accounts_sdk:  # acc_sdk is a TeamTalkUserAccount object
                ...
                if hasattr(acc_sdk, "username"):
                    ...
                    logger.warning(
                        "TeamTalk UserAccount object %s (type: %s) does not have 'username' attribute.",
                        acc_sdk,
                        type(acc_sdk),
                    )

        logger.info(
            "Fetched %s accounts from TeamTalk server using list_user_accounts.",
            len(user_accounts_display),
        )

    except Exception:
        logger.exception(
            "Error fetching TeamTalk accounts using list_user_accounts:"
        )
        try:
            await callback_query.message.edit_text(
                _(
            ...
            logger.debug(
                "Failed to edit message for TT account fetching error: %s", e_edit
            )
            await callback_query.message.answer(
                _(
                    "Could not connect to the TeamTalk server to get the list of accounts."
    ...
    try:
        await callback_query.message.edit_text(
            message_text,
            reply_markup=builder.as_markup() if user_accounts_display else None,
        )
    except Exception as e:
        logger.warning(
            "Failed to edit message for TT account list (maybe no change or too old): %s",
            e,
        )
...
async def prompt_delete_tt_account_handler(
    callback_query: types.CallbackQuery, callback_data: AdminTTAccountsCallback
) -> None:
...
async def confirm_delete_tt_account_handler(
    ...
    if not tt_username:
        logger.error(
            "confirm_delete_tt_account_handler: tt_username missing in callback_data during delete confirmation."
        )
        await callback_query.answer(
        ...
        try:
            ...
            logger.debug(
                "Failed to edit message for missing tt_username on confirm: %s", e_edit
            )
        return
    ...
    if (
        ...
        logger.warning(
            "confirm_delete_tt_account_handler: TeamTalk instance not available or not connected for deleting %s.",
            tt_username,
        )
        connection_error_text = _(
        ...
        try:
            await callback_query.message.edit_text(
                connection_error_text, reply_markup=None
            )
        except Exception as e_edit:
            logger.debug(
                "Failed to edit message for TT connection error on confirm: %s", e_edit
            )
            await callback_query.message.answer(
                connection_error_text, reply_markup=None
            )  # Send as new if edit fails
    ...
    try:
        ...
        if deletion_command_sent:  # Returns True on success as per pytalk docs
            ...
            logger.info(
                "TeamTalk user '%s' deletion command successfully processed by bot for admin %s. Waiting for server event for actual ban.",
                tt_username,
                callback_query.from_user.id,
            )
        else:
            ...
            logger.warning(
                "TeamTalk user '%s' deletion command returned False for admin %s without raising an exception.",
                tt_username,
                callback_query.from_user.id,
            )
            final_message = _(
                ...
                error="TeamTalk command indicated failure but no specific error.",
            )
            await callback_query.answer(final_message, show_alert=True)

    except PermissionError as e:  # Specific exception from pytalk for permission issues
        logger.exception(
            "Permission error deleting TeamTalk user '%s' by admin %s:",
            tt_username,
            callback_query.from_user.id,
        )
        final_message = _(
        ...
        logger.exception(
            "Value error (e.g., user not found) deleting TeamTalk user '%s' by admin %s:",
            tt_username,
            callback_query.from_user.id,
        )
        final_message = _(
            ...
            tt_username=tt_username, error=f"Invalid request/user not found: {e}"
        )
        await callback_query.answer(final_message, show_alert=True)
    except Exception:  # Catch-all for other unexpected errors from the TeamTalk library or other issues
        logger.exception(
            "Generic error deleting TeamTalk user '%s' by admin %s:",
            tt_username,
            callback_query.from_user.id,
        )
        final_message = _(
    ...
    try:
        ...
        logger.debug(
            "Failed to edit message after TT delete confirmation for user %s: %s",
            tt_username,
            e_edit,
        )
...
async def generate_deeplink_handler(
    ...
    if message.from_user.id not in settings.admin_ids:
        logger.warning(
            "User %s (not an admin) tried to use /generate.", message.from_user.id
        )
    ...
    try:
        ...
        if not hasattr(bot, "username") or not bot.username:
            ...
            await message.reply(
                _(
                    "Internal error: bot username is not available. Please contact support."
                )
            )
            return
